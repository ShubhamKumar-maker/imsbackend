...
public class SheduleInterviewDaoImpl implements SheduleInterviewDao {
	...
	public SheduleInterview savaData(SheduleInterview sheduleinterview) {
		...
		namedParameterJdbcTemplate.update(sql, srcMap, holder,
				new String[] { SheduleInterviewTableConstants.SHEDULEINTERVIEWID });
		return sheduleinterview;
	}

	@Override
	public List<SheduleInterview> getallData() {
		String sqlquery = "select * from sheduleInterview join employee on sheduleInterview.panelID=employee.username join candidate on candidate.candidateID=sheduleInterview.candidateID join skillsSet on skillsSet.skillID=sheduleInterview.skillID where interviewDate>=curdate();";
		return jdbctemplate.query(sqlquery, (resultSet, rowNum) -> {
			...
			sheduleinterview.setCandidatename(resultSet.getString(CandidateTableConstant.CANDIDATNAME));
			sheduleinterview.setSkill(resultSet.getString(SheduleInterviewTableConstants.SKILL));
			sheduleinterview.setPanelname(resultSet.getString(SheduleInterviewTableConstants.EMPLOYEENAME));
			return sheduleinterview;
		});
	}
